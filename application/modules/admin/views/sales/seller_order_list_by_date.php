      ...
      <div class="row">
        <div class="table-responsive">  
          <table class="table table-hover" id="table1" cellspacing="0" width="100%">
            ...
            <tbody>
              <tr>
                <td colspan="10" class="text-right"><a href="<?php echo base_url().'admin/commission/seller_incentive_list_by_id/'.$seller_id.'/?all_cleared=1';?>" class="btn btn-success">Update all as Paid</a></td>
              </tr>
              <?php
              $counter=0;
              $all_total_amount=0;
              ...
              if (!empty($order_list)) { 
                ...
                foreach ($order_list as $row):
                  ...
                  if($row->commission_paid>0)
                    $commission = $row->commission_paid;
                  else{
                    $commission = $this->general_model->getValue('commission_amount','tbl_commission','sales_id="'.$row->id.'"');
                    $commission_data = array();
                    $commission_data = array('commission_paid' => $commission);
                    $this->db->where('id',$row->id);
                    $this->db->update('tbl_sales',$commission_data);
                  }
                  $incentive_total+=$commission;
                ?>
                <tr>
                  ...
                  <td><?php echo date("d F Y", strtotime($row->sales_date));?><br><?php echo $row->commission_payment_status;?></td>
                  ...
                  <td class="text-right">
                    ...
                    <?php echo $row->commission_payment_status;?>
                    <?php
                      if(isset($_GET['all_cleared']) && $_GET['all_cleared']=='1' && $row->commission_payment_status=='Not Paid'){
                        echo "Updated as cleared.";
                        $data_payment_status = array();
                        $where = array();
                        $data_payment_status = array('commission_payment_status' => 'Paid');
                        //$where = array('id',$row->id);
                        $this->general_model->update('tbl_sales', $data_payment_status,'id="'.$row->id.'"');
                      }
                    ?>
                  </td>
