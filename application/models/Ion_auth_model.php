...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		// Users table.
		$active = 0;
		$accesstype = $additional_data['accesstype'];
		if($accesstype==2){
			$active = 1;
		}
		else{
	...
	/**
	 * [dropdown_array description]
	 * @param  boolean $id_field   [description]
	 * @param  boolean $list_field [description]
	 * @return [type]              [description]
	 */
	public function dropdown_array($id_field = FALSE, $list_field = FALSE,$table,$where='')
	{
		if($where){
			$this->db->where($where);
		}
		$customers =$this->db->get($table);
		$result = array();


		if ($id_field === FALSE || $list_field === FALSE) return $result;

		$array = $customers->result_array();
	...
	public function checkbox($id_field = FALSE, $list_field = FALSE,$table,$where='')
	{
		if($where){
			$this->db->where($where);
		}
